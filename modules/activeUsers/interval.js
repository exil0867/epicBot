...
exports.run = async () => {
  ...
  usersListQuery.map((user) => {
    ...
    if ((beingActiveSince !== null) && (dateNow.diff(moment(beingActiveSince), 'days') >= process.env.DAYS_TO_GET_ACTIVE_ROLE) && (hasActiveRoleSince === null)) {
    ...
    if ((beingActiveSince !== null) && (dateNow.diff(moment(lastTimeBeingActive), 'days') >= process.env.DAYS_TO_LOSE_ACTIVE_ROLE)) {
